...
import edu.tjrac.swant.common.DialogFrag4Input
...
class TorrentActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!isPermissionGranted(Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
            appCompatRequestPermissions(arrayOf(Manifest.permission.WRITE_EXTERNAL_STORAGE), 0)
    ...
    private fun subscribeToViewComponents() {
        buttonAddTorrent.setOnClickListener {
            ...
            var dialog = DialogFrag4Input(inputType = null)
            dialog.pos = View.OnClickListener {
                var content = dialog.getInput()
                if (TextUtils.isEmpty(content)) {
                    //                    T.show("请输入参数")
                } else {
                    val tabFragment = createTabWithUri(Uri.parse(dialog.et?.text.toString()))
                    torrentSessionPagerAdapter.addTab(tabFragment)
                }
            }
            dialog?.show(supportFragmentManager, "torrent link")
     ...
     * a torrent file.
     */
    @Suppress("unused") private fun startFilePickerActivity() {
        val intent: Intent
